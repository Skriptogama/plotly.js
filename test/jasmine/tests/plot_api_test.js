...
describe('Test plot api', function () {
    ...
    describe('Plotly.newPlot', function () {
        ...
        it('accepts gd, data, layout, and config as args', function (done) {
            Plotly.newPlot(gd, [{ x: [1, 2, 3], y: [1, 2, 3] }], { width: 500, height: 500 }, { editable: true })
                .then(function () {
    ...
    describe('Plotly.relayout', function () {
        ...
        it('updates autosize/width/height correctly', function (done) {
            ...
            Plotly.newPlot(gd, [{ y: [1, 3, 2] }])
                .then(assertSizeAndThen(543, 432, true, 'initial', { autosize: false }))
                .then(assertSizeAndThen(543, 432, false, 'autosize false with no sizes', { autosize: true }))
                .then(assertSizeAndThen(543, 432, true, 'back to autosized', { width: 600 }))
                .then(assertSizeAndThen(600, 432, false, 'explicit width causes explicit height', { width: null }))
                .then(assertSizeAndThen(543, 432, true, 'removed width', { height: 500, width: 700 }))
                .then(assertSizeAndThen(700, 500, false, 'explicit height and width', { autosize: true }))
    ...
    describe('Plotly.relayout subroutines switchboard', function () {
        ...
        it('should trigger replot (but not recalc) when changing attributes that affect axis length/range', function () {
            ...
            for (attr in axLayoutEdits) {
                ...
                Plotly.relayout(gd, attr, axLayoutEdits[attr]).then(checkAttr);
            }
        });
        ...
        it('should trigger minimal sequence for cartesian axis range updates', function () {
            ...
            specs.forEach(function (s) {
                ...
                _assert(
                    ['Plotly.', s[0], '(gd, ', JSON.stringify(s[1][0]), ', ', JSON.stringify(s[1][1]), ')'].join('')
                );
            });
        });
    ...
    describe('Plotly.restyle subroutines switchboard', function () {
        ...
        [
            ...
            it(
                "should clear calcdata when restyling 'zmin' and 'zmax' on " + gd.data.type + ' traces',
                function (done) {
                    mockDefaultsAndCalc(gd);
                    plotApi._doPlot.calls.reset();
        ...
        [
            ...
            it(
                "should not clear calcdata when restyling 'zmin' and 'zmax' on " + gd.data.type + ' traces',
                function (done) {
                    mockDefaultsAndCalc(gd);
                    plotApi._doPlot.calls.reset();
                    Plotly.restyle(gd, 'zmin', 0)
        ...
        it('ignores invalid trace indices', function (done) {
            ...
            Plotly.restyle(gd, { type: 'scatter', 'marker.color': 'red' }, [1]).then(done, done.fail);
        });
            ...
            Plotly.newPlot(gd, [
                ...
                .then(function () {
                    return Plotly.restyle(
                        gd,
                        { 'marker.line.cmax': mlcmax1, 'marker.line.colorscale': mlcscl1 },
                        null,
                        [1]
                    );
                })
                ...
                .then(function () {
                    return Plotly.restyle(
                        gd,
                        { 'marker.line.cauto': true, 'marker.line.autocolorscale': true },
                        null,
                        [1]
                    );
                })
    ...
    describe('Plotly.deleteTraces', function () {
        ...
        it('should work when indices are negative', function () {
            var expectedData = [{ name: 'a' }, { name: 'b' }, { name: 'c' }];
        ...
        it('should work when multiple traces are deleted', function () {
            var expectedData = [{ name: 'b' }, { name: 'c' }];
    ...
    describe('Plotly.moveTraces should', function () {
        ...
        it('accept integers in place of arrays', function () {
            var expectedData = [{ name: 'b' }, { name: 'a' }, { name: 'c' }, { name: 'd' }];
        ...
        it('handle unsorted currentIndices', function () {
            var expectedData = [{ name: 'd' }, { name: 'a' }, { name: 'c' }, { name: 'b' }];
        ...
        it('work when newIndices are undefined.', function () {
            var expectedData = [{ name: 'b' }, { name: 'c' }, { name: 'd' }, { name: 'a' }];
        ...
        it('accept negative indices.', function () {
            var expectedData = [{ name: 'a' }, { name: 'c' }, { name: 'b' }, { name: 'd' }];
    ...
    describe('cleanData & cleanLayout', function () {
        ...
        it('should cleanup annotations / shapes refs', function () {
            ...
            var layout = {
                annotations: [null, { xref: 'x02', yref: 'y1' }],
                shapes: [{ xref: 'y', yref: 'x' }, null, { xref: 'x03', yref: 'y1' }]
            };
    ...
    describe('Plotly.update should', function () {
        ...
        it('clear calcdata on data + axis updates w/o extending current gd.data', function (done) {
            ...
            Plotly.update(gd, traceUpdate, layoutUpdate)
                .then(function () {
                    expect(data).toBe(gd.data);
                    expect(layout).toBe(gd.layout);
                    expect(calcdata).not.toBe(gd.calcdata);
        ...
        it('call doLegend on legend updates', function (done) {
            Plotly.update(gd, {}, { showlegend: true })
                .then(function () {
