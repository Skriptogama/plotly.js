...
describe('Visible rangesliders', function () {
    ...
    it('should automargin correctly with a top or bottom x axis', function (done) {
        ...
        function assertTop(hasExtra) {
            ...
            // With side:'same' (default), slider follows the top axis → no bottom push
            expect(gd._fullLayout._size.b).toBeWithin(80, 5);
        }
    ...
    it('should restore full-extent handles after Plotly.resetRangeSliders', function (done) {
        var maskMin, maskMax, sliderWidth;

        plotMock().then(function () {
            maskMin = getRangeSliderChild(2);
            maskMax = getRangeSliderChild(3);
            sliderWidth = gd._fullLayout.xaxis.rangeslider._width;

            // initial state: slider covers full data range, no masked area
            expect(+maskMin.getAttribute('width')).toBeWithin(0, TOL);
            expect(+maskMax.getAttribute('width')).toBeWithin(0, TOL);

            // zoom into a subset — left handle moves away from x=0
            return Plotly.relayout(gd, 'xaxis.range', [10, 20]);
        })
            .then(function () {
                // confirm the left mask has grown (left grabber is no longer at x=0)
                expect(+maskMin.getAttribute('width')).toBeGreaterThan(TOL);

                return Plotly.resetRangeSliders(gd);
            })
            .then(function () {
                // after reset: axis is back to full data range
                expect(gd.layout.xaxis.range).toBeCloseToArray([0, 49], -0.5);

                // and the slider handles are back at the edges
                expect(+maskMin.getAttribute('width')).toBeWithin(0, TOL);
                expect(+maskMax.getAttribute('width')).toBeWithin(0, TOL);
            })
            .then(done, done.fail);
...
describe('Rangeslider handleDefaults function', function () {
    ...
    it('should default side to \'same\'', function () {
        var layoutIn = { xaxis: { rangeslider: {} } };
        var layoutOut = { xaxis: {} };

        _supply(layoutIn, layoutOut, 'xaxis');
        expect(layoutOut.xaxis.rangeslider.side).toEqual('same');
    });

    it('should accept side \'opposite\'', function () {
        var layoutIn = { xaxis: { rangeslider: { side: 'opposite' } } };
        var layoutOut = { xaxis: {} };

        _supply(layoutIn, layoutOut, 'xaxis');
        expect(layoutOut.xaxis.rangeslider.side).toEqual('opposite');
    });

    it('should set defaults for a yaxis rangeslider', function () {
        var layoutIn = { yaxis: { rangeslider: {} } };
        var layoutOut = { yaxis: {} };
        var expected = {
            visible: true,
            autorange: true,
            ...
            bordercolor: '#444',
            side: 'same',
            _input: layoutIn.yaxis.rangeslider
        };

        _supply(layoutIn, layoutOut, 'yaxis');
        expect(layoutOut.yaxis.rangeslider).toEqual(jasmine.objectContaining(expected));
    });

    it('should default side to \'same\' for yaxis rangeslider', function () {
        var layoutIn = { yaxis: { rangeslider: {} } };
        var layoutOut = { yaxis: {} };

        _supply(layoutIn, layoutOut, 'yaxis');
        expect(layoutOut.yaxis.rangeslider.side).toEqual('same');
    });

    it('should accept side \'opposite\' for yaxis rangeslider', function () {
        var layoutIn = { yaxis: { rangeslider: { side: 'opposite' } } };
        var layoutOut = { yaxis: {} };

        _supply(layoutIn, layoutOut, 'yaxis');
        expect(layoutOut.yaxis.rangeslider.side).toEqual('opposite');
    });

    it('should return early for yaxis if *visible: false*', function () {
        var layoutIn = { yaxis: { rangeslider: { visible: false, range: [0, 10] } } };
        var layoutOut = { yaxis: { rangeslider: {} } };

        _supply(layoutIn, layoutOut, 'yaxis');
        expect(layoutOut.yaxis.rangeslider).toEqual(jasmine.objectContaining({ visible: false }));
    });
});
...
describe('Rangeslider yaxis vertical (xaxis opp-axis options)', function () {
    it('should set xaxis opts when one xaxis is present', function () {
        var mock = {
            data: [
                { x: [1, 2], y: ['a', 'b'], orientation: 'h', type: 'bar' }
            ],
            layout: {
                xaxis: { anchor: 'y' },
                yaxis: { rangeslider: {} }
        ...
        expect(mock._fullLayout.yaxis.rangeslider.xaxis).toEqual(jasmine.objectContaining({ rangemode: 'match' }));
    });

    it('should honor xaxis range user settings', function () {
        var mock = {
            data: [
                { x: [1, 2], y: ['a', 'b'], orientation: 'h', type: 'bar' }
            ],
            layout: {
                ...
                yaxis: {
                    rangeslider: {
                        xaxis: { rangemode: 'fixed', range: [0, 5] }
                    }
                }
            }
        };
        ...
        expect(mock._fullLayout.yaxis.rangeslider.xaxis).toEqual(jasmine.objectContaining({ rangemode: 'fixed', range: [0, 5] }));
    });
});
...
describe('Rangeslider anchored axes fixedrange', function () {
    ...
    it('should set yaxis fixedrange to *true* when yaxis has a vertical rangeslider', function () {
        var mock = {
            data: [
                { x: [1, 2], y: [1, 2], orientation: 'h' }
            ],
            layout: {
                xaxis: { anchor: 'y' },
                yaxis: { rangeslider: {} }
        ...
        expect(mock._fullLayout.yaxis.rangeslider.visible).toBe(true);
        expect(mock._fullLayout.yaxis.fixedrange).toBe(true);
    });

    it('should set the anchored xaxis fixedrange to *true* when yaxis has a vertical rangeslider', function () {
        var mock = {
            data: [
                { x: [1, 2], y: [1, 2], orientation: 'h' }
            ],
        ...
        expect(mock._fullLayout.yaxis.rangeslider.visible).toBe(true);
        expect(mock._fullLayout.xaxis.fixedrange).toBe(true);
    });
});
...
describe('Rangeslider categorical axis positions: all 4 axis sides × 2 slider sides', function () {
    var gd;
    beforeEach(function () { gd = createGraphDiv(); });
    afterEach(destroyGraphDiv);

    // ── xaxis bottom ─────────────────────────────────────────────────────────
    it('xaxis bottom + rangeslider side same → slider is at bottom', function (done) {
        var mockData = require('../../image/mocks/range_slider_xaxis_bottom_same');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider on bottom pushes _size.b well above default (80)
            expect(gd._fullLayout._size.b).toBeGreaterThan(100);
            // nothing on top – top margin stays at the layout.margin.t default
            expect(gd._fullLayout._size.t).toBe(100);
        }).then(done, done.fail);
    });

    it('xaxis bottom + rangeslider side opposite → slider is at top', function (done) {
        // mock sets margin.t:20 so the slider must visibly expand the top margin
        var mockData = require('../../image/mocks/range_slider_xaxis_bottom_opposite');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider on top forces _size.t above the reduced default (20)
            expect(gd._fullLayout._size.t).toBeGreaterThan(40);
            // (bottom margin is pushed independently by axis labels, not the slider)
        }).then(done, done.fail);
    });

    // ── xaxis top ────────────────────────────────────────────────────────────
    it('xaxis top + rangeslider side same → slider is at top', function (done) {
        var mockData = require('../../image/mocks/range_slider_xaxis_top_same');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider + top-axis labels push _size.t above the default 100
            expect(gd._fullLayout._size.t).toBeGreaterThan(100);
            // nothing on bottom – bottom margin stays at layout.margin.b default
            expect(gd._fullLayout._size.b).toBe(80);
        }).then(done, done.fail);
    });

    it('xaxis top + rangeslider side opposite → slider is at bottom', function (done) {
        var mockData = require('../../image/mocks/range_slider_xaxis_top_opposite');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider on bottom pushes _size.b above the default 80
            expect(gd._fullLayout._size.b).toBeGreaterThan(100);
        }).then(done, done.fail);
    });

    // ── yaxis left ───────────────────────────────────────────────────────────
    it('yaxis left + rangeslider side same → slider is at left', function (done) {
        var mockData = require('../../image/mocks/range_slider_yaxis_left_same');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider + left-axis depth push _size.l well above default (80)
            expect(gd._fullLayout._size.l).toBeGreaterThan(100);
            // nothing on right – right margin stays at layout.margin.r default
            expect(gd._fullLayout._size.r).toBe(80);
        }).then(done, done.fail);
    });

    it('yaxis left + rangeslider side opposite → slider is at right', function (done) {
        var mockData = require('../../image/mocks/range_slider_yaxis_left_opposite');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider on right pushes _size.r above the default 80
            expect(gd._fullLayout._size.r).toBeGreaterThan(100);
            // left has only short tick-label depth so stays below 100
            expect(gd._fullLayout._size.l).toBeLessThan(100);
        }).then(done, done.fail);
    });

    // ── yaxis right ──────────────────────────────────────────────────────────
    it('yaxis right + rangeslider side same → slider is at right', function (done) {
        var mockData = require('../../image/mocks/range_slider_yaxis_right_same');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider + right-axis depth push _size.r well above default (80)
            expect(gd._fullLayout._size.r).toBeGreaterThan(100);
            // nothing on left – left margin stays at layout.margin.l default
            expect(gd._fullLayout._size.l).toBe(80);
        }).then(done, done.fail);
    });

    it('yaxis right + rangeslider side opposite → slider is at left', function (done) {
        var mockData = require('../../image/mocks/range_slider_yaxis_right_opposite');
        Plotly.newPlot(gd, mockData).then(function () {
            // slider on left pushes _size.l above the default 80
            expect(gd._fullLayout._size.l).toBeGreaterThan(100);
            // right has only short tick-label depth so stays below 100
            expect(gd._fullLayout._size.r).toBeLessThan(100);
        }).then(done, done.fail);
    });
});
