...
describe('Test scatter', function () {
    ...
    describe('supplyDefaults', function () {
        ...
        it('should coerce tension for cardinal line shape', function () {
            traceIn = {
                x: [1, 2, 3],
                y: [1, 2, 3],
                line: { shape: 'cardinal' }
        ...
        it('should coerce supplied tension value for cardinal shape', function () {
            traceIn = {
                x: [1, 2, 3],
                y: [1, 2, 3],
                line: { shape: 'cardinal', tension: 0.2 }
            };
            traceOut = { visible: true };
            supplyDefaults(traceIn, traceOut, defaultColor, layout);
            expect(traceOut.line.tension).toBe(0.2);
        });

        it('should not coerce tension for non-cardinal shapes', function () {
            traceIn = {
                x: [1, 2, 3],
                y: [1, 2, 3],
                line: { shape: 'linear', tension: 0.2 }
            };
            traceOut = { visible: true };
            supplyDefaults(traceIn, traceOut, defaultColor, layout);
            expect(traceOut.line.tension).toBeUndefined();
        });

        it('should coerce alpha for catmull-rom line shape', function () {
            traceIn = {
                x: [1, 2, 3],
                y: [1, 2, 3],
        ...
        it('should coerce supplied alpha value for catmull-rom shape', function () {
            traceIn = {
                x: [1, 2, 3],
                y: [1, 2, 3],
                line: { shape: 'catmull-rom', alpha: 0.8 }
            };
            traceOut = { visible: true };
            supplyDefaults(traceIn, traceOut, defaultColor, layout);
            expect(traceOut.line.alpha).toBe(0.8);
        });

        it('should accept monotone and natural as valid line shapes', function () {
            ['monotone', 'natural'].forEach(function (shape) {
                traceIn = { x: [1, 2, 3], y: [1, 2, 3], line: { shape: shape } };
                traceOut = { visible: true };
                supplyDefaults(traceIn, traceOut, defaultColor, layout);
                expect(traceOut.line.shape).toBe(shape);
            });
        });
    });
