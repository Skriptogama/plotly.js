...
describe('scattergl line shapes', function () {
    ...
    it('should coerce cardinal tension and catmull-rom alpha', function () {
        ...
        supplyDefaults({
            ...
            y: [0, 1, 0],
            mode: 'lines',
            line: { shape: 'catmull-rom' }
        }, traceOut, '#444', {});

        expect(traceOut.line.shape).toBe('catmull-rom');
        expect(traceOut.line.alpha).toBe(0.5);
    });

    it('should sample smooth line shapes into extra GL vertices', function () {
        var baseTrace = {
            visible: true,
            mode: 'lines',
            line: {
        ...
        var positions = [0, 0, 10, 25, 20, 5, 30, 30];

        ['spline', 'cardinal', 'catmull-rom', 'monotone', 'natural'].forEach(function (shape) {
            var line = Lib.extendFlat({}, baseTrace.line, { shape: shape, smoothing: 1, tension: 0.5, alpha: 0.5 });
            var result = convert.linePositions(null, Lib.extendFlat({}, baseTrace, { line: line }), positions);

            expect(result.positions.length).toBeGreaterThan(positions.length, shape);
            expect(result.positions[0]).toBe(positions[0], shape + ' x0');
            expect(result.positions[1]).toBe(positions[1], shape + ' y0');
            expect(result.positions[result.positions.length - 2]).toBe(positions[positions.length - 2], shape + ' xN');
            expect(result.positions[result.positions.length - 1]).toBe(positions[positions.length - 1], shape + ' yN');
        });
    });
